...
async function main() {
    ...
    console.log("\n=== 5. Company Pays and Accepts Student (Escrow) ===");
    try {
        const payResult = await submissionService.paySubmission(submissionId, companyUser.id, {
            cardHolderName: "Sinerji Test Company",
            cardNumber: "5890040000000016", // Iyzico Sandbox Card
            expireMonth: "12",
            expireYear: "30",
            cvv: "123"
        }, companyUser.email);
        console.log("✅ Payment successful! Funds locked in escrow. Status:", payResult.status);
    } catch (e: any) {
        console.log("❌ Workflow Error Encountered During Payment!");
        console.log("Message:", e.message || e);
        return;
    }
